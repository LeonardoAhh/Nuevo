...
const BRAND = {
    ...
    warn: [249, 115, 22] as [number, number, number],
...
const STATUS_LABEL: Record<AptitudStatus, string> = {
    ...
    no_apto: "No Apto",
    pendiente: "Espera",
    pendiente_examen: "Examen",
    en_revision: "Revisión",
    bloqueado_examen: "Penalizado",
}
...
function computeKpis(empleados: EmpleadoPromocion[]) {
    ...
    for (const emp of habilitados) {
        ...
        else if (st === "pendiente" || st === "pendiente_examen") pendientes++
        else revision++
    }
...
function statusRGB(s: AptitudStatus): [number, number, number] {
    ...
    if (s === "pendiente" || s === "pendiente_examen") return BRAND.warn
    if (s === "en_revision") return BRAND.info
...
export async function exportPromocionesExcel(empleados: EmpleadoPromocion[], meta?: ExportMeta) {
    ...
    rows.forEach((r, idx) => {
        ...
        const map: Record<AptitudStatus, [string, string]> = {
            ...
            pendiente: [HEX.neutralFill, HEX.neutralText],
            pendiente_examen: [HEX.warnFill, HEX.warnText],
            en_revision: [HEX.infoFill, HEX.infoText],
            bloqueado_examen: [HEX.badFill, HEX.badText],
        }
