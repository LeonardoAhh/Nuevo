  ...
  const generarExamen = useCallback(
    async (empleado: EmpleadoBusqueda, transicionKey: TransicionKey) => {
      ...
      try {
        ...
        const { data: examenGuardado, error: insertErr } = await supabase
          .from("historial_examenes")
          .insert({
            empleado_id: empleado.id,
            departamento: empleado.departamento ?? "DESCONOCIDO",
            categoria_actual: actual,
        ...
        if (insertErr) {
          console.error("Error al guardar el examen:", insertErr)
          notify.error("Error al guardar historial del examen")
        } else {
          notify.success("Examen guardado correctamente")
        }

        setExamen({
          id: examenGuardado?.id,
          empleado,
          transicion: {
