...
export function useCapacitacion() {
      ...
      const ecRecords = preview.rawRecords
        ...
        .map(r => {
          ...
          return {
            ...
            calificacion: (() => {
              const c = r['CALIFICACIÓN'] ? parseInt(r['CALIFICACIÓN']) : null;
              if (c === null || isNaN(c)) return null;
              if (c > 10) return Math.round(c / 10);
              return Math.min(10, Math.max(0, c));
            })(),
          }
        })
  ...
  const migrateGrades = async (updates: { id: string; calificacion: number | null }[]) => {
    try {
      const { error } = await supabase
        .from('employee_courses')
  ...
  const addCoursesToEmployee = async (employeeId: string, employeeCourses: { course_id: string; course_name: string; fecha_aplicacion: string | null; calificacion: number | null }[]): Promise<{ success: boolean; error?: string }> => {
    setImporting(true)
    setImportError(null)
    try {
  ...
  const createCourse = async (
    ...
    try {
      const { error } = await supabase
        .from('courses')
        .insert([{ name: name.trim(), tipo, duration_hours: durationHours }])
      if (error) throw new Error(error.message)
      return { success: true }
    } catch (err) {
  ...
  // ── Actualizar curso (nombre / tipo / duración) ──────────────────────────

  const updateCourse = async (
    courseId: string,
    data: { name?: string; tipo?: string | null; duration_hours?: number | null }
  ): Promise<{ success: boolean; error?: string }> => {
    try {
      const payload: { name?: string; tipo?: string | null; duration_hours?: number | null } = {}
